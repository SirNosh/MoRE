...
def train_step(input_ids, labels, fabric, state, running_loss, running_ppl, cfg):
    ...
    def tightly_scoped_fwd_bwd(model, input_ids, positions, labels, mask):
        with fabric.no_backward_sync(model, enabled=state["is_accumulating"]):
            ...
            # Optional MoE auxiliary loss (load-balancing) and routing metrics
            aux_weight_str = os.getenv("MOE_AUX_LOSS_WEIGHT", "0.01")  # Default to 0.01 for MoE-Huginn
            try:
                aux_weight = float(aux_weight_str)
            except ValueError:
                aux_weight = 0.01
            ...
            if aux_weight > 0.0:
                ...
                # Check for MoE metrics from our SharedExpertMoE implementation
                if hasattr(base_model, "get_moe_metrics"):
                    moe_metrics = base_model.get_moe_metrics()
                else:
                    # Fallback to checking transformer.moe for backward compatibility
                    moe_layer = getattr(getattr(base_model, "transformer", base_model), "moe", None)
                    if moe_layer is not None and hasattr(moe_layer, "get_last_metrics"):
                        moe_metrics = moe_layer.get_last_metrics()
                
                if moe_metrics is not None and isinstance(moe_metrics.get("load_balance_loss"), torch.Tensor):
                    total_loss = total_loss + aux_weight * moe_metrics["load_balance_loss"]

            fabric.backward(total_loss / cfg.gradient_accumulation_steps, model=model)

            # Record MoE metrics for logging
            if moe_metrics is not None:
                try:
                    state["metrics"]["moe_aux_weight"] = torch.as_tensor(aux_weight, device=fabric.device)
                    state["metrics"]["moe_lb_loss"] = moe_metrics["load_balance_loss"].detach()
                    state["metrics"]["moe_routing_entropy"] = moe_metrics["routing_entropy"].detach()
                    state["metrics"]["moe_active_experts"] = (moe_metrics["expert_counts"] > 0).sum().detach()
                    
                    # Enhanced MoE metrics for comprehensive monitoring
                    if "expert_utilization" in moe_metrics:
                        state["metrics"]["moe_expert_utilization"] = moe_metrics["expert_utilization"].detach()
                    
                    if "expert_preference" in moe_metrics:
                        state["metrics"]["moe_expert_preference"] = moe_metrics["expert_preference"].detach()
                        state["metrics"]["moe_max_expert_attention"] = moe_metrics["max_expert_attention"].detach()
                        state["metrics"]["moe_min_expert_attention"] = moe_metrics["min_expert_attention"].detach()
                        state["metrics"]["moe_attention_spread"] = moe_metrics["attention_spread"].detach()
                    
                    # Router collapse detection metrics
                    if "expert_counts" in moe_metrics:
                        expert_counts = moe_metrics["expert_counts"]
                        total_tokens = expert_counts.sum()
                        if total_tokens > 0:
                            # Calculate Gini coefficient for expert utilization (measure of inequality)
                            ...
                            state["metrics"]["moe_gini_coefficient"] = gini_coefficient.detach()
                            
                            # Calculate percentage of tokens routed to top expert
                            top_expert_usage = expert_counts.max() / total_tokens
                            state["metrics"]["moe_top_expert_usage"] = top_expert_usage.detach()
                            
                            # Calculate percentage of tokens routed to bottom expert
                            bottom_expert_usage = expert_counts.min() / total_tokens
                            state["metrics"]["moe_bottom_expert_usage"] = bottom_expert_usage.detach()
                            
                            # Calculate expert diversity (how many experts are actually used)
                            active_experts = (expert_counts > 0).sum()
                            expert_diversity = active_experts / expert_counts.shape[0]
                            state["metrics"]["moe_expert_diversity"] = expert_diversity.detach()
                    
                    # Load balancing quality metrics
                    if "load_balance_loss" in moe_metrics:
                        # Normalize load balance loss for better interpretation
                        n_experts = moe_metrics.get("expert_counts", torch.tensor(8)).shape[0]
                        normalized_lb_loss = moe_metrics["load_balance_loss"] / n_experts
                        state["metrics"]["moe_normalized_lb_loss"] = normalized_lb_loss.detach()
                        
                        # Router collapse warnings
                        if "router_collapse_warning" in moe_metrics:
                            state["metrics"]["moe_router_collapse_warning"] = moe_metrics["router_collapse_warning"]
                        
                        if "low_expert_diversity_warning" in moe_metrics:
                            state["metrics"]["moe_low_expert_diversity_warning"] = moe_metrics["low_expert_diversity_warning"]
                    
                    # Per-expert detailed metrics
                    if "expert_preference" in moe_metrics:
                        expert_preference = moe_metrics["expert_preference"]
                        for i in range(expert_preference.shape[0]):
                            state["metrics"][f"moe_expert_{i}_preference"] = expert_preference[i].detach()
                    
                    # Attention variance for stability monitoring
                    if "attention_variance" in moe_metrics:
                        state["metrics"]["moe_attention_variance"] = moe_metrics["attention_variance"].detach()
                    
                    # Top expert usage percentage for collapse detection
                    if "top_expert_usage_pct" in moe_metrics:
                        state["metrics"]["moe_top_expert_usage_pct"] = moe_metrics["top_expert_usage_pct"].detach()
                    
                except Exception as e:
                    fabric.print(f"Warning: Failed to record MoE metrics: {e}")
                    # Continue training even if metrics recording fails

            return total_loss.detach(), outputs["log_ppl"].detach()
