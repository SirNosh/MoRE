...
class RecurrentGPT(torch.nn.Module):
    def __init__(
        ...
        # Optional MoE layer applied to token embeddings after prelude
        if getattr(config, "num_experts", 0) and config.num_experts > 0:
            from .moe_layers import MoELayer  # local import to keep optional
            ...
            moe_norm = config.Norm(config.n_embd, eps=config.norm_eps)
        else:
            moe = torch.nn.Identity()
            moe_norm = torch.nn.Identity()
